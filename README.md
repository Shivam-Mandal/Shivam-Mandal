<h1 align="center">Hi 👋, I'm Shivam Mandal</h1>

<h3 align="center">
🚀 Full Stack MERN Developer | Docker & VPS Deployment | C++ Programmer
</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?lines=Full+Stack+MERN+Developer;React+%26+Node.js+Developer;Docker+%26+VPS+Deployment;500%2B+LeetCode+Problems;Always+Learning+New+Things&center=true&width=750&height=50">
</p>

---

<img align="right" alt="coding" width="350"
src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif">

# 👨‍💻 About Me

- 🔭 Building scalable and production-grade MERN applications
- 🌱 Exploring advanced backend systems & DevOps
- 🚀 Experienced in Docker, VPS Hosting & Production Deployment
- 💬 Ask me about React.js, Node.js, MongoDB, Express.js & APIs
- ⚡ Solved 500+ DSA problems on LeetCode
- 🏆 Passionate about scalable architecture & performance optimization
- 📫 Reach me at: **shivammandal83404@gmail.com**

---

# 🌐 Connect With Me

<p align="center">

<a href="https://linkedin.com/in/shivam-mandal-4a2528230" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="https://github.com/Shivam-Mandal" target="_blank">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<a href="https://leetcode.com/u/Shivam_Mandal" target="_blank">
  <img src="https://img.shields.io/badge/LeetCode-000000?style=for-the-badge&logo=leetcode&logoColor=orange" />
</a>

<a href="mailto:shivammandal83404@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

</p>

---

# 🚀 Tech Stack

## 💻 Programming Languages

<p align="left">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="45"/>&nbsp;&nbsp;
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45"/>&nbsp;&nbsp;
</p>

---

## ⚛️ Frontend Development

<p align="left">
  <img src="https://skillicons.dev/icons?i=react" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=tailwind" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=bootstrap" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=javascript" height="50"/>&nbsp;&nbsp;
</p>

<p align="left">
  <img src="https://reactnative.dev/img/header_logo.svg" height="50"/>&nbsp;&nbsp;
</p>

---

## 🛠 Backend Development

<p align="left">
  <img src="https://skillicons.dev/icons?i=nodejs" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=express" height="50"/>&nbsp;&nbsp;
</p>

---

## 🗄 Databases

<p align="left">
  <img src="https://skillicons.dev/icons?i=mongodb" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=mysql" height="50"/>&nbsp;&nbsp;
</p>

---

## ☁️ DevOps & Deployment

<p align="left">
  <img src="https://skillicons.dev/icons?i=docker" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=git" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=github" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=postman" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=androidstudio" height="50"/>&nbsp;&nbsp;
  <img src="https://skillicons.dev/icons?i=linux" height="50"/>&nbsp;&nbsp;
</p>

### 🚀 Deployment Experience

- 🐳 Dockerized full-stack MERN applications
- ☁️ Deployed production applications on VPS servers
- 🌐 Experience with Hostinger VPS hosting
- ⚡ Frontend deployment using Vercel
- 🔥 Backend deployment using Render
- 🔧 Managed environment variables & production APIs
- 🚀 Configured scalable deployment workflows

---

# 📊 GitHub Stats

<p align="center">

<img height="180em" src="https://github-readme-stats-sigma-five.vercel.app/api?username=Shivam-Mandal&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00F7FF&icon_color=00F7FF"/>

</p>

<p align="center">

<img width="49%" src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=Shivam-Mandal&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00F7FF"/>

<img width="49%" src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=Shivam-Mandal&theme=tokyonight"/>

</p>

---

# 🐍 Contribution Snake

<p align="center">
  <img src="https://raw.githubusercontent.com/Shivam-Mandal/Shivam-Mandal/output/snake.svg" alt="Snake animation" />
</p>

---

# 🏆 GitHub Achievements

<p align="center">
<img src="https://github-profile-trophy.vercel.app/?username=Shivam-Mandal&theme=onedark&row=1&column=7"/>
</p>

---

# 🚀 Featured Projects

## 🔹 Worker & Resource Tracking System
### MERN Stack | Client Project

- Production-grade workforce & resource management platform
- Implemented role-based authentication & authorization
- Real-time worker and task allocation system
- Scalable REST API architecture for enterprise usage

---

## 🔹 Food Delivery Application
### MERN Stack | Stripe | JWT | Tailwind CSS

- Full-stack food ordering platform
- Integrated Stripe payment gateway
- Secure JWT authentication system
- Dynamic routing & order management
- Developed 10+ scalable REST APIs

---

## 🔹 TypeRace – Multiplayer Typing Game
### React.js | Node.js | Socket.IO

- Multiplayer real-time typing game
- Dynamic score tracking system
- Matchmaking support for multiple users
- Achieved sub 3-second latency

---

## 🔹 Movie Application
### React Native | TMDB API

- Cross-platform mobile application
- Integrated 500+ movie records
- Real-time API data fetching using Axios
- Smooth multi-screen navigation system

---

# 🏅 Achievements

- ✅ Solved 500+ Problems on LeetCode
- 🌍 Global Rank 140,000+ on LeetCode
- 🥉 3rd Prize in Project Expo for IoT Smart Irrigation System
- 🎨 2nd Place in Bihar Saptah Painting Competition
- 🏆 Awarded by Chief Minister Nitish Kumar

---

# 💡 Developer Quote

<p align="center">
<i>"First, solve the problem. Then, write the code."</i>
</p>

---

<p align="center">
⭐ From <a href="https://github.com/Shivam-Mandal">Shivam Mandal</a>
</p>
